...
class Income_groups_cash_out_ubp extends Admin_Controller {
	private
		$_admin_account_data = NULL,
		$_transaction_type_id = "txtype_cashout2";
	...
	public function index($page = 1) {
		...
		$this->_data['add_url']	 		= base_url() . "income-groups-cash-out-ubp/new";
	...
	public function new() {
		...
		$base_form_url				= base_url() . "income-groups-cash-out-ubp/new";
		...
		$this->_data['title']  	= "Cash Out (UBP) - New Group Member";
	...
	public function update($group_id) {
		...
		$this->_data['notification'] 	= $this->session->flashdata('notification');
		$this->_data['form_url']		= base_url() . "income-groups-cash-out-ubp/update/{$group_id}";
		...
		$this->_data['listing'] = $this->table_listing('', $results, count($results));
		$this->_data['title']  	= "Cash Out (UBP) - Update Group";
		$this->set_template("income_groups/form", $this->_data);
	}

	public function delete($group_id) {
		$this->_data['notification'] 	= $this->session->flashdata('notification');
		$this->_data['form_url']		= base_url() . "income-groups-cash-out-ubp/delete/{$group_id}";
		...
		if ($_POST) {
			...
			if ($confirmation == "DELETE") {
				...
				redirect(base_url() . "income-groups-cash-out-ubp");
			}
		}

		$this->_data['title']  	= "Cash Out (UBP) - DELETE GROUP - ID: {$group_id}";
