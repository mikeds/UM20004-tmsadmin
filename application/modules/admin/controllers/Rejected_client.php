...
class Rejected_client extends Admin_Controller {
		
	public function after_init() {
		$this->set_scripts_and_styles();
		
		$this->load->model("admin/disapproval_reason_types_model", "disapproval_types");
        $this->load->model("admin/client_disapproval_model", "client_disapproval");
	...
	public function index($page = 1) {
		...
		$inner_joints = array(
			array(
				...
				'condition'		=> 'disapproval_reason_types.disapproval_reason_type_id = client_rejected.disapproval_reason_type_id'
			)
		);
        $where = array();

		$total_rows = $this->client_disapproval->get_count(
			$where,
			array(),
			$inner_joints
		);
		...
		$data = $this->client_disapproval->get_data(
			$select,
			$where,
			array(),
			$inner_joints,
		...
		$this->_data['title']  = "Rejected Client";
		$this->set_template("rejected_client/list", $this->_data);
	}
}
